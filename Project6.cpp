...
double findMidpoint(double x, double low_guess, double high_guess);

/* return the smallest of the elements in array x[]
 * there are n elements in x[] (x[0].. x[n-1])
 */
int minIt(int x[], int n) {
    int min = x[0];  // temp min
    int i;

    for(i = 1; i < n; i++){ // go through whole array and check if current min is smaller than most current element being checked
        if(x[i] < min) min = x[i];
    }
    return min;
}
...
int minRec1(int x[], int n) {
    ...
    // Find a minimum
    int remaining_min = minRec1(x+1, n-1);

    if(x[0] < remaining_min){
        return x[0];
    } else return remaining_min;
}
...
int minRec2(int x[], int n) {
    // compare two halves , n/2 and (n+1)/2 (think about odd lengths)
    // store minimum into a variable

    // Base Case, one element in array
    if(n == 1) return x[0];

    // Cutting the arrays in half
    int first_half = minRec2(x, n/2);
    int second_half = minRec2(x+(n/2),(n+1)/2); // n+1 in the case of an odd length, so it includes the extra, last element
...
double sqrtIt(double x, double low_guess, double high_guess) {
    ...
    while(true){
        ...
        if(increment == 1e-15) return root - increment;
        increment /= 10;
        increment *= -1;
 ...
 * accuracy.
 */
double sqrtRec(double x, double low_guess, double high_guess) { double error = 1e-15;
    // Base Case: the low and high guesses are so close that the midpoint must be the answer
    //            accurate to 15 decimal places
    double midpoint = findMidpoint(x,low_guess,high_guess);
    if((high_guess - low_guess)/midpoint < error) return midpoint;

    if(midpoint*midpoint < x) return sqrtRec(x,midpoint,high_guess);
    if(midpoint*midpoint > x) return sqrtRec(x,low_guess,midpoint);
}
...
int strCompare2(char* str1, char* str2) {// "!"  ""
    ...
    if(whatLetter(str1[0]) == -1) return strCompare2(++str1, str2);
    if(whatLetter(str2[0]) == -1) return strCompare2(str1, ++str2);

    // If there's an alphabetical difference
...
int solveMazeRec(int row, int col) {
    // Putting down bread crumb
    maze[row][col] = 2;

    // Base Case: We start at the end of the maze!
    if (row == MATRIX_SIZE - 1) return 1;

    // Checking all four cardinal directions
    // UP, only do it if you're NOT on the top
   if (row != 0) {
        if (maze[row-1][col] == 0)   // Check for empty space
            if (solveMazeRec(row-1, col)) return 1;
    }
    // RIGHT
    if(maze[row][col+1] == 0)
        if(solveMazeRec(row,col+1)) return 1;
    // DOWN
    if(maze[row+1][col] == 0)
        if(solveMazeRec(row+1,col)) return 1;
    // LEFT
    ...
    // No way through, pick up crumb and return false
    maze[row][col] = 0;
    return 0;
}
...
Martian change(int cents) {
    ...
    // Base Case, there's nothing to add up
    if(cents == 0) {
        return base;
    }
    ...
    if(cents >= 1){
        small = change(cents - 1);
        small.pennies += 1;
    }

    if(cents >= 5){
        medium = change(cents - 5);
        medium.nicks += 1;
    }

    if(cents >= 12){
        large = change(cents - 12);
        large.dodeks += 1;
    }

    int total1 = small.pennies + small.nicks + small.dodeks;
    int total2 = medium.pennies + medium.nicks + medium.dodeks;
    int total3 = large.pennies + large.nicks + large.dodeks;

    if(cents >= 1) min = small;
    if(cents >= 5){
        if(total2 < total1) min = medium;
    }
    if(cents >= 12){
        int temptotal = min.pennies + min.nicks + min.dodeks;
        if(total3 < temptotal) min = large;
    }
...
Martian change(int cents, int nick_val, int dodek_val) {
    ...
    if(cents >= 1){
        small = change(cents - 1, nick_val, dodek_val);
        small.pennies += 1;
    }

    if(cents >= nick_val){
        medium = change(cents - nick_val, nick_val, dodek_val);
        medium.nicks += 1;
    }

    if(cents >= dodek_val){
        large = change(cents - dodek_val, nick_val, dodek_val);
        large.dodeks += 1;
    }
